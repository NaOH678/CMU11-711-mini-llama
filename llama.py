...
class RMSNorm(torch.nn.Module):
    ...
    def _norm(self, x):
        ...
        """
        # todo
        # a_sum_squared = torch.square(x).sum(dim=-1, keepdim=True)
        # 一开始忘了求均值
        a_squared = torch.mean(x**2, dim=-1, keepdim=True)
        rms = torch.sqrt(a_squared + self.eps)
        norm = x / rms
        return norm
...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        '''
        # todo
        batch, seqlen, _, _ = query.shape

        # 理解score矩阵的含义：
        # Scores 矩阵的每个值表示一个 token 对另一个 token 的注意力分数。具体来说：
        #   对于第 i 个 token，scores[i, j] 表示第 i 个 token 对第 j 个 token 的注意力分数。
        #   注意力分数越高，表示第 i 个 token 越关注第 j 个 token。
        scores = query @ key.transpose(-2, -1)

        # 为什么不要mask？？？ 删去掩码之后通过了测试
        # tril = torch.tril(torch.ones(seqlen, seqlen))
        # scores = scores.masked_fill(tril == 0, float('-inf'))

        scores = F.softmax(scores/ math.sqrt(self.head_dim), dim=-1)
        atten_weight = self.attn_dropout(scores)

        output = atten_weight @ value    #    这里不需要transpose，因为forward中会进行transpose
    ...
    def forward(
        ...
        # llama2 的 rope 是针对 query 和 key 的
        ...
        # make heads into a batch dimension, compute dot product for each head
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0):
        ...
        for _ in range(max_new_tokens):
            ...
            if temperature == 0.0:
                ...
                '''
                new_probs = F.softmax(logits / temperature, dim = -1)
                idx_next = torch.multinomial(new_probs, num_samples=1)
                # idx_next = idx_next.unsqueeze(-1) # 此处不需要unsqueeze(-1)
